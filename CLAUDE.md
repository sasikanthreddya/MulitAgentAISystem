...
Defined in [`.mcp.json`](.mcp.json) at the project root (git-tracked). Each server loads its own credentials directly from a file in this repo — `mcp-atlassian` via its native `--env-file` flag, the two REST servers via a `dotenv-cli` wrapper — so nothing depends on which app launched Claude Code or what's in its process environment. See `.env.example` / `.env.twilio.example`.
...
| `rest-api` | Generic HTTP client for whatever API a story targets | Reads `.env`; `REST_BASE_URL` defaults to Restful-Booker, repoint locally, no code change |
| `rest-api-twilio` | Dedicated REST client, pinned to Twilio | Reads its own `.env.twilio` so Twilio's Basic-auth creds never leak into the generic `rest-api` server |
